    ...
    : RoutableViewModelBase
{
    ...
    private async Task StartSingleplayerAsync()
    {
        ...
        try
        {
            ...
            WarnIfBepInExModsDetected(NitroxUser.GamePath);
    ...
    private static void WarnIfBepInExModsDetected(string gameDir)
    {
        ...
        string bepInExRoot = Path.Combine(gameDir, "BepInEx");
        if (!Directory.Exists(bepInExRoot))
        {
            return;
        }

        string[] modFolders = ["plugins", "patchers"];
        List<string> detectedMods = [];

        foreach (string folder in modFolders)
        {
            string folderPath = Path.Combine(bepInExRoot, folder);
            if (Directory.Exists(folderPath))
            {
                detectedMods.AddRange(Directory.EnumerateFiles(folderPath, "*.dll", SearchOption.AllDirectories));
            }
        }
        ...
        Log.Warn($"BepInEx mods detected ({detectedMods.Count}): {string.Join(", ", detectedMods.Select(Path.GetFileNameWithoutExtension))}");
        string warningMessage = detectedMods.Count == 1
            ? "检测到BepInEx模组，Nitrox多人游戏不支持模组，可能导致不稳定"
            : $"检测到{detectedMods.Count}个BepInEx模组，Nitrox多人游戏不支持模组，可能导致不稳定";
        LauncherNotifier.Warning(warningMessage);
    }
}
